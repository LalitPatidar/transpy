...
#-------------------------------------------------------------------------------------------------#
N = 5000                        # Number of orientations or directions of approach of bath gas
...
sigma = []
epsilon = []
file_out = species_name +'_' + bath_gas + '_' + combining_rule +'.out'
...
if combining_rule == 'GM':    
    sigma_ii = 2.0*sigma_ij - sigma_bath
    epsilon_ii = (epsilon_ij**2.0/epsilon_bath)*cm2K

if combining_rule == 'HM':
    sigma_ii = 2.0*sigma_ij - sigma_bath
    temp = 2.0/epsilon_ij - 1.0/epsilon_bath
    epsilon_ii = cm2K*1.0/temp
    #epsilon_ii = ((epsilon_ij*epsilon_bath)/(2*epsilon_bath-epsilon_ij))*cm2K
    
if combining_rule == 'SPM':
...
with open(file_out,'a') as File:
    File.writelines('--------------------------------------------------------------\n')
    File.writelines('Pure species Lennard Jones parameters based on %s and using %s\n'%(bath_gas,combining_rule))
